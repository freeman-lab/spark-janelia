...
qsub @{common_job_args} -l h_rt=@{job_max_run_seconds} \
  -N ${WORKER_READY_JOB_NAME} -pe 1 \
  -j -o "@{run_logs_dir}/11-verify-workers-ready.log" \
...
qsub @{common_job_args} -l h_rt=@{job_max_run_seconds} \
  -N ${WORKER_JOB_NAME} -t 1-@{max_worker_nodes} -hold_jid ${URL_JOB_NAME} -pe @{worker_slots} \
  -j -o "@{run_logs_dir}/worker-\$TASK_ID-launch.log" \
  "@{run_scripts_dir}/03-launch-worker.sh"

qsub @{common_job_args} -l h_rt=@{job_max_run_seconds} \
  -N ${DRIVER_JOB_NAME} -hold_jid ${WORKER_READY_JOB_NAME}  -pe @{driver_slots} \
  -j -o "${DRIVER_LOG}" \
